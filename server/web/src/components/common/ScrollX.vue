...
// Обновить статус переполнения содержимого
...
// Обработчик прокрутки с использованием колесика мыши
function handleMouseWheel(e) {
  const { wheelDelta } = e
  const wrapperWidth = wrapper.value?.offsetWidth
  const contentWidth = content.value?.offsetWidth
  /**
   * @wheelDelta Значение прокрутки: >0 — сдвиг вправо, <0 — сдвиг влево
   * @translateX Текущее значение translateX содержимого
...
// Сброс положения translateX при переполнении
...
function handleScroll(x, width) {
  ...
  // Если x меньше минимального видимого значения
  if (x < -translateX.value + 150) {
    translateX.value = -(x - 150)
    resetTranslateX(wrapperWidth, contentWidth)
  }

  // Если x больше максимального видимого значения
