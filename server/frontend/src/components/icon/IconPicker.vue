...
const choosed = ref(props.value) // Выбранное значение
const icons = ref(iconData)
// const icons = ref(iconData.filter((icon) => icon.includes(choosed.value))) // Список доступных иконок
...
<template>
  <div class="w-full">
    <NPopover trigger="click" placement="bottom-start">
      <template #trigger>
        <NInput v-model:value="choosed" placeholder="Введите название иконки" @update:value="filterIcons">
          <template #prefix>
      ...
      <template #footer>
        Для большего количества иконок посетите
